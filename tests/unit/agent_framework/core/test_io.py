...
from src.agent_framework.core.io import create_input_from_request, parse_agent_response
...
def test_parse_agent_response_bad_json_returns_raw():
    ...
    # It should fallback to wrapping the raw text in the schema if possible (robustness)
    event = SimpleNamespace(content=SimpleNamespace(parts=[SimpleNamespace(text="not-json")]))
    out = parse_agent_response(SimpleOutput, event)
    
    # New behavior: creates SimpleOutput(value='not-json')
    assert isinstance(out, SimpleOutput)
